...
interface BusQrScannerModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: (log: AttendanceLog, entityName: string) => void;
}

const BusQrScannerModal: React.FC<BusQrScannerModalProps> = ({ isOpen, onClose, onSuccess }) => {
  const { user } = useAuth();
  const [scanResult, setScanResult] = useState<{ type: 'success' | 'error' | 'loading', message: string } | null>(null);
  const readerId = `qr-reader-bus-${Math.random().toString(36).slice(2, 11)}`;
  ...
  const handleQrCode = async (decodedText: string) => {
    try {
      ...
      const { log, entityName } = await api.busMarkAttendanceByQr(user.token, decodedText);
      const statusText = log.status === 'BUS_IN' ? 'Boarded Vehicle' : 'De-boarded Vehicle';
      setScanResult({ type: 'success', message: `Success! ${entityName} - ${statusText}` });

      setTimeout(() => {
        onSuccess(log, entityName);
      }, 1500);
    } catch (err) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-sm">
        ...
        <div className="p-6 relative text-center">
             <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">Point the camera at a student QR code.</p>
...
export default BusQrScannerModal;